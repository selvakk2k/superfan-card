        ...
        <div class="body-container">
          ...
          <div class="presets-container" style="display: flex; flex-direction: column; gap: 12px;">
            ${c.length>0?B`
              ...
              <div class="pill-grid">
                ${c.map(t=>{const e=t.toLowerCase().includes("speed adjust"),i=Boolean(r&&"none"!==r&&!r.toLowerCase().includes("speed adjust"));return B`
                    <button 
                      class="pill-btn ${r===t?"active":""}" 
                      ?disabled=${e&&i}
                      style="${e&&i?"opacity: 0.5;":""}"
      ...
      <ha-card style="${c}" class="gh-full-card">
        ...
        <div class="gh-slider-container">
          <input type="range" class="gh-slider" 
            ...
            .value="${s}" 
            ?disabled=${!i}
            @change=${t=>{const e=t.target;let i=parseInt(e.value,10);if(3===o){i=[0,33,66,100].reduce((t,e)=>Math.abs(e-i)<Math.abs(t-i)?e:t)}else if(5===o){i=[0,20,40,60,80,100].reduce((t,e)=>Math.abs(e-i)<Math.abs(t-i)?e:t)}else{const t=100/o;i=Math.round(i/t)*t}this._setSpeed(Math.round(i)),e.value=Math.round(i).toString()}}
          >
        </div>

        ${n.length>0||a.length>0?B`
          <div class="gh-pill-grid">
            ${n.map(t=>{const e=t.toLowerCase().includes("speed adjust"),i=Boolean(r&&"none"!==r&&!r.toLowerCase().includes("speed adjust"));return B`
                <button 
                  class="gh-pill ${r===t?"active":""}" 
                  ?disabled=${e&&i}
                  style="${e&&i?"opacity: 0.5;":""}"
                  @click=${()=>this._setPreset(t)}
                >
    ...
    `}_renderCompact(t,e,i,s,r,o){const n=i?r||`${s}%`:"Off";return B`
      <ha-card class="compact-card" @click=${()=>this._expanded=!0}>
        ...
        <div class="compact-footer">
          <button class="compact-action-btn" ?disabled=${!i} @click=${t=>{t.stopPropagation(),this._cycleSpeed(s,o,-1)}}>
