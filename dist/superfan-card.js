        ...
        <div class="body-container">
          ...
          <div class="presets-container">
            ${g.length>0?B`
              ...
              <div class="pill-grid">
                ${g.map(e=>{const t=e.toLowerCase().includes("speed adjust"),i=Boolean(d&&"none"!==d&&!d.toLowerCase().includes("speed adjust")),s=ue[e]||"";let o=e;s&&(o+=` — ${s}`),a?c?t&&i&&(o="Deactivate current preset to adjust speed"):o="Turn on fan to activate preset":o="Device is offline";const n=s||o;return B`
                    <button
                      class="pill-btn ${d===e&&c?"active":""} ${!a||!c||t&&i?"disabled":""}"
                      title="${o}"
                      @touchstart=${()=>this._handleTouchStart(n)}
                      @touchend=${()=>this._handleTouchEnd()}
                      @touchcancel=${()=>this._handleTouchEnd()}
                      @click=${s=>{if(this._isLongPress)return this._isLongPress=!1,s.preventDefault(),void s.stopPropagation();a?c?t&&i?this._showToast("Deactivate current preset to adjust speed"):this._setPreset(e):this._showToast("Turn on fan to activate preset"):this._showToast("Device is offline")}}
                    >
            ...
            ${m.length>0?B`
              ...
              <div class="pill-grid">
                ${m.map(e=>{const t=ue[e]||`Automatically turns off after ${e}`;let i=e;t&&(i+=` — ${t}`),a?c||(i="Turn on fan to set timer"):i="Device is offline";const s=t||i;return B`
                    <button
                      class="pill-btn ${d===e&&c?"active":""} ${a&&c?"":"disabled"}"
                      title="${i}"
                      @touchstart=${()=>this._handleTouchStart(s)}
                      @touchend=${()=>this._handleTouchEnd()}
                      @touchcancel=${()=>this._handleTouchEnd()}
